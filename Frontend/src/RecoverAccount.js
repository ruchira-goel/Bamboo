...
export default class RecoverAccount extends Component {
  ...
  sendEmail = () => {
    ...
    if (!expression.test(String(email).toLowerCase())) {
      Alert.alert('Not Valid', 'Ensure that the email is valid.', [
        {text: 'OK'},
      ]);
      return;
    }
    fetch(
      Platform.OS === 'android'
        ? `${Constants.URL.android}/User/recoverAccount?email=${email}`
        : `${Constants.URL.ios}/User/recoverAccount?email=${email}`,
    )
      .then(res => res.json())
      .then(data => {
        console.log(data);
